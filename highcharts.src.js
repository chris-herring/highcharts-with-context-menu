		...
		function drop() {
			if (mouseTracker.selectionMarker) {
				var selectionData = {
                        ...
                        // next to the pointer
                        clientX: (e && e.originalEvent) ? e.originalEvent.clientX : null,
                        clientY: (e && e.originalEvent) ? e.originalEvent.clientY : null
                         */
					},
			...
			if (chart) { // it may be destroyed on mouse up - #877
                ...
                //chart.rightClick = hasDragged = false;
			}
            ...
            if (!hasDragged) {
                if (hoverPoint) {
                    ...
                    extend(hoverPoint, {
                        ...
                        mousePageX: e.pageX || e.clientX,
                        mousePageY: e.pageY || e.clientY
                    });
		...
		if (!li) { // generate it once, later move it
			...
			item.legendGroup.on('mouseover', function () {
                ...
                .on('contextmenu', function (e) {
                    ...
                    var strLegendContextMenu = 'legendContextMenu',
                        fnLegendContextMenu = function () {
                        },
                        args = {
                            mousePageX: e.pageX || e.clientX,
                            mousePageY: e.pageY || e.clientY
                    ...
                    // click the name or symbol
                    if (item.firePointEvent) { // point
                        item.firePointEvent(strLegendContextMenu, args, fnLegendContextMenu);
                    } else {
                        fireEvent(item, strLegendContextMenu, args, fnLegendContextMenu);
                    }
                });
